...
# arr_1 = [10, 6, 3, 2, 5 ]
# arr_1_id = arr_1.object_id
# filter_out!(arr_1) { |x| x.odd? }
# p arr_1     # [10, 6, 2]
# p arr_1_id == arr_1.object_id

# arr_2 = [1, 7, 3, 5 ]
# arr_2_id = arr_2.object_id
# filter_out!(arr_2) { |x| x.odd? }
# p arr_2     # []
# p arr_2_id == arr_2.object_id

# arr_3 = [10, 6, 3, 2, 5 ]
# arr_3_id = arr_3.object_id
# filter_out!(arr_3) { |x| x.even? }
# p arr_3     # [3, 5]
# p arr_3_id == arr_3.object_id

# arr_4 = [1, 7, 3, 5 ]
# arr_4_id = arr_4.object_id
# filter_out!([1, 7, 3, 5 ]) { |x| x.even? }
# p arr_4 # [1, 7, 3, 5]
# p arr_4_id == arr_4.object_id
...
def proctition(array, &prc)
    ...
    array.each do |ele|
        prc.call(ele) ? left << ele : right << ele
    end
...
def selected_map!(array, testPrc, mutatePrc)
    array.map! do |el|
        if testPrc.call(el)
            mutatePrc.call(el)
        else
...
def proc_suffix(str, hash)
    ...
    new_words = words.map do |word|
        newWord = word
        hash.each{|prc, suffix|newWord += suffix if prc.call(word)}
        newWord
    end
...
# hash1 = { contains_a => 'ly', three_letters => 'o' }
# p proc_suffix('dog cat', hash1 )   # "dogo catlyo"
...
def procipher(string, actionHash)
    words = string.split
    mappedWords = words.map do |word|
        wordStore = word.dup
        actionHash.each do |k, v|
            word = v.call(word) if k.call(wordStore)
        end
        word
    end
    mappedWords.join(" ")
...
def picky_procipher(sentence, cipherHash)
    words = sentence.split
    encryptedWords = words.map do |word|
        cipherHash.each do |key, prc|
            wordInstance = word.dup
            if key.call(wordInstance)
                word = prc.call(word)
                break
            end
        end
