...
class MetaCorgiSnacks
  ...
  #   snack_types = ["bone", "kibble", "treat"]
  #   if snack_types.include?(name.to_s)
  #     info = @snack_box.send("get_#{name}_info".to_s, @box_id)
  #     tastiness = @snack_box.send("get_#{name}_tastiness".to_s, @box_id)
  #     description = "#{name.capitalize}: #{info}: #{tastiness}"
  #     puts (tastiness.to_i > 30 ? "* #{description}" : description )
  #   else
  #     super
  #   end
  # end
